...
export function useUserSession() {
    ...
    /**
     * Record a vote on a poll question. Shape:
     *     pollVotes[pollId][questionId] = optionId
     * If user clicks the same option again, we delete the entry (toggle off,
     * matches the server behaviour).
     */
    const recordPollVote = useCallback((pollId, questionId, optionId) => {
        setProfile((curr) => {
            const allVotes = { ...(curr?.pollVotes || {}) };
            const pollVotes = { ...(allVotes[pollId] || {}) };
            if (pollVotes[questionId] === optionId) {
                delete pollVotes[questionId]; // toggle off
            } else {
                pollVotes[questionId] = optionId;
            }
            allVotes[pollId] = pollVotes;
            const next = { ...(curr || {}), pollVotes: allVotes };
            saveProfile(next);
            return next;
        });
    }, []);
    ...
    const updateDisplayName = useCallback((name) => {
        setProfile((curr) => {
            const next = { ...(curr || {}), displayName: name };
            saveProfile(next);
            return next;
        });
    }, []);
    ...
    const resetEverything = useCallback(() => {
        ...
        if (typeof window !== "undefined") {
            try {
                window.localStorage.removeItem("discuss-demo-notif-read");
            } catch {}
            window.location.href = "/";
        }
    }, []);

    const toggleSaveTopic = useCallback((entry) => {
        // entry: { id, title, industry }
        setProfile((curr) => {
            const list = Array.isArray(curr?.savedTopics) ? curr.savedTopics : [];
            const exists = list.some((s) => s.id === entry.id);
            const next = {
                ...(curr || {}),
                savedTopics: exists
                    ? list.filter((s) => s.id !== entry.id)
                    : [{ ...entry, savedAt: new Date().toISOString() }, ...list].slice(0, 100),
            };
            saveProfile(next);
            return next;
        });
